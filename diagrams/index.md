...
Diagram role names: `--ink --body --secondary --dim --accent --accent-light --accent-2 --teal --warm --positive --emphasis --bg --card --card-accent --border`. These are the diagrams' internal vocabulary; `tokenize-svg --palette` maps your profile's tokens (`--text-primary`, `--text-body`, `--accent`, `--accent-2`, …) onto them. (`--accent-2` is mapped everywhere but lightly used out of the box: only `roadmap-feature-mapping` renders with it. It's there for re-skins and your own diagrams.)
